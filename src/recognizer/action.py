...
import recognizer.actionbase as actionbase
...
class Follow(object):
    ...
    """Determines who the user wants to follow"""

    def __init__(self, say, keyword, voice_in_q):
        self.say = say
        self.keyword = keyword
        self.voice_in_q = voice_in_q

    def run(self, voice_command):
        to_follow = voice_command.replace(self.keyword, '', 1)
        self.voice_in_q.put({'follow': to_follow})
        self.say('Following' + to_follow)
...
class Track(object):
    ...
    def __init__(self, say, voice_in_q):
        self.say = say
        self.voice_in_q = voice_in_q

    def run(self, voice_command):
        self.voice_in_q.put({'Track': True})
        self.say('Track objects')
...
class Detect(object):
    ...
    def run(self, voice_command):
        self.voice_in_q.put({'detect': True})
        self.say('Detecting objects')


def make_actor(say, voice_in_q, voice_out_q):
    ...
    actor.add_keyword(_('my name'), SpeakAction(say, _('You are Raouf Muhamedrahimov')))
    actor.add_keyword(_('where am i'),
                      SpeakAction(say, _('You are at FAB lab Namibia, the happiest place on Earth')))

    actor.add_keyword(_('detect object'),
                      Detect(say, voice_in_q))
    
    actor.add_keyword(_('start tracking'), voice_in_q.put({'track': True}))
    
    actor.add_keyword(_('stop tracking'), voice_out_q.put({'stop track': True}))
    
    actor.add_keyword(_('follow'),
                      Follow(say, _('follow'), voice_in_q))
    actor.add_keyword(_('stop tracking'), voice_out_q.put({'stop follow': True}))
