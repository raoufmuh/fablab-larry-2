...
import recognizer.audio as audio
import recognizer.action as action
import recognizer.i18n as i18n
import recognizer.speech as speech
import recognizer.tts as tts
...
def main(voice_in_q, voice_out_q):
    ...
    with recorder:
        do_recognition(args, recorder, recognizer, player, voice_in_q, voice_out_q)


def do_recognition(args, recorder, recognizer, player, voice_in_q, voice_out_q):
    """Configure and run the recognizer."""
    say = tts.create_say(player)

    actor = action.make_actor(say, voice_in_q, voice_out_q)
...
class RecognizerThread(threading.Thread):
    def __init__(self, voice_in_q, voice_out_q):
        threading.Thread.__init__(self, name='Voice Recognition Thread')
        self.voice_in_q = voice_in_q
        self.voice_out_q = voice_out_q

    def run(self):
        main(self.voice_in_q, self.voice_out_q)
